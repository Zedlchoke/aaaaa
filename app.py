...
# ==========================================
# PHIÊN BẢN: LOẠI BỌ MÀU (Fix lỗi aRGB) - 17/07/2026
# - Loại bỏ toàn bộ màu nền để tránh lỗi colors must be aRGB hex values
# - Giữ lại border + alignment + wrap text
# - Logic tách Mua/Bán + Sheet giải thích P1-P8 vẫn giữ
...
def format_excel_sheet(ws, is_doichieu=False):
    ...
    # Header: chỉ bold + căn giữa + border (không màu)
    for cell in ws[1]:
        cell.font = Font(bold=True)
        cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
        cell.border = thin_border

    # Data rows: chỉ border + wrap
    ...
    try:
        ...
        for row in items:
            clean_items.append({
                ...
                'TTVND_TT': parse_amt_to_float(row.get('TTVND_TT', 0)),
...
def process_bank_data(file_saoke, file_master, path_save_doichieu, path_save_saokemoi, user_stop_str, file_muavao=None, file_banra=None, progress_callback=None, priority="name_first", enabled_ps=None, custom_order=None):
    ...
    for w in user_stops:
        ...
        if w_norm:
            dynamic_stops.append(r'\b' + re.escape(w_norm) + r'\b')

    if progress_callback:
        progress_callback(5, "Đang đọc file Excel...")
    ...
    for idx, row in df_bank.iterrows():
        if progress_callback and idx % 20 == 0:
            progress_callback(15 + (idx / total_rows) * 65, f"Đối soát dòng {idx+1}...")
        ...
        if pd.isna(diengiai_goc) or diengiai_goc.strip() == "" or diengiai_goc.lower().strip() == "nan":
            all_matches.append({
                "THỨ TỰ DÒNG GỐC": thu_tu_dong, "DIỄN GIẢI GỐC": "", "TÊN QUÉT ĐƯỢC TRẠNG DIỄN GIẢI": "",
                "HÓA ĐƠN QUÉT ĐƯỢC TRẠNG DIỄN GIẢI": "", "HỢP ĐỒNG QUÉT ĐƯỢC TRẠNG DIỄN GIẢI": "",
                "SỐ TIỀN QUÉT ĐƯỢC TRẠNG DIỄN GIẢI": "", "TÊN MATCH ĐƯỢC TRẠNG FILE ĐỐI TƯỢNG PHÁP NHÂN": "",
    ...
    # Sheet giải thích P1-P8 (không màu)
    ...
    for r in range(12, 16):
        ws_legend.cell(row=r, column=1).font = Font(italic=True)

    wb_doichieu.save(path_save_doichieu)

    # Cập nhật file SaoKê (không tô màu vàng nữa)
...
def process_update_saoke(file_sk_old, file_dc_edited, path_save, progress_callback=None):
    if progress_callback:
        progress_callback(5, "Đang đọc file đối chiếu đã chỉnh sửa tay...")
    try:
    ...
    if progress_callback:
        progress_callback(25, f"Tìm thấy {len(update_map)} dòng cần cập nhật...")

    if progress_callback:
        progress_callback(35, "Đang mở file Sao kê gốc...")
    try:
    ...
    if progress_callback:
        progress_callback(90, f"Đã cập nhật {updated_count} dòng...")
    wb.save(path_save)
    if progress_callback:
        progress_callback(100, "Cập nhật thành công! (SửA TAY)")
